...
# --- 2. LOGIK & CACHE ---
geolocator = Nominatim(user_agent="integral_pro_v58_final")
...
# --- FUNKTIONEN (MIT AUTO-KORREKTUR & CRS-FIX) ---
def verarbeite_strasse(strasse):
    ...
    try:
        ...
        # 2. Fallback: Unscharfe Suche (Fuzzy)
        if gdf.empty:
        ...
        if not gdf.empty:
            ...
            gdf = gdf.to_crs(epsg=4326) # Wichtig für Folium Zoom
...
with col_title:
    ...
    st.markdown("Automatisierte Sortierung — **V5.8**")
...
if st.session_state.run_processing and strassen_liste:
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        ...
        for i, future in enumerate(futures):
            ...
            status_text.text(f"🔍 Gefunden: {res.get('name', 'Suche...')}")
...
if st.session_state.ort_sammlung:
    if st.session_state.fehler_liste:
        with st.expander("⚠️ Nicht gefundene Einträge (trotz Korrektur-Versuch)"):
    ...
    for ort, items in st.session_state.ort_sammlung.items():
        ...
        for item in items:
            ...
            folium.GeoJson(
                ...
                tooltip=f"Gefunden als: {item['name']} (Input: {item['original']})"
            ).add_to(fg)
        fg.add_to(m)
    ...
    st.download_button(label="📥 Karte speichern", data=html_string, file_name="Ergebnis.html", mime="text/html")
