...
# --- 1. SETUP & CONFIG ---
SERIAL_NUMBER = "SN-029-GOLD3002-INSTANT-FIX"
...
st.caption(f"Status: {SERIAL_NUMBER} | Straßen geladen: {len(st.session_state.saved_manual_streets)}")

with st.container(border=True):
    # Der Key muss stabil sein, aber wir erzwingen den State-Sync
    up = st.file_uploader("*.txt Dateien hochladen", type=["txt"], accept_multiple_files=True, key="uploader_instant")
    ...
    if up:
        ...
        if new_raw:
            # Kombiniere alte und neue Daten ohne Duplikate
            combined = sorted(list(set(st.session_state.saved_manual_streets + new_raw)))
            
            # Prüfen ob wirklich neue Straßen dazugekommen sind
            if len(combined) != len(st.session_state.saved_manual_streets):
                ...
                # WICHTIG: Sofortiger Rerun, damit die Analyse-Sektion die Daten sieht
                st.rerun()
    ...
    # Dieser Button ist jetzt sofort aktiv, sobald der State oben aktualisiert wurde
    if c_btn2.button("🔥 ANALYSE STARTEN", type="primary", use_container_width=True):
        if not st.session_state.saved_manual_streets:
            st.error("❌ Keine Straßen geladen! Bitte erst Datei hochladen.")
        else:
